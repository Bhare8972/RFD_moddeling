...
if __name__=='__main__':
    table_in=array_input( binary_input("../tables/ionization_losses_RML") )
    ...
    ##plot like lehtinen!
    plt.plot(electron_energy/1000.0, electron_SP/10, label='table electron SP') ##stopping power in units of MeV m^2 kg^-1??
    ...
    ##plot like dwyer
    plt.plot(electron_energy, electron_SP*1000*density, label='table electron SP') ##stopping power in units of KeV /cm
    ...
    ##plot native units
    plt.plot(electron_mom_sq, electron_SP_native, label='table electron SP')
    plt.plot(electron_mom_sq[1:], beth_calculation[1:], label='beth formula')
