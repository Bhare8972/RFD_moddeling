...
def read_particle_history(fname="./particle_history_output"):
    fin=binary_input(fname)
    electrons={}
    
    while not fin.at_end():
        command=fin.in_short()
        if command==1: ## a  new electron
            ID=fin.in_int()
            electrons[ID]=electron_history(ID, fin)

        elif command==2: ##update existing electron
            ID=fin.in_int()
            electrons[ID].update(fin)

        elif command==3: ##remove existing electron
            ID=fin.in_int()
            electrons[ID].remove(fin)
        elif command==4: ##at end!
            break
        else:
...
def num_electrons_per_T(electrons, num_seeds=1.0, max_T=None):
    ...
    for e in electrons.values():
        ...
        for i in range(len(electrons_each_T)):
...
if __name__=='__main__':
    ...
    print( len(electron_data), "particles")

    plot_particles_XYZ(electron_data)
    plot_particles_XZ(electron_data)
    plot_particles_TIMEYZ(electron_data)
    plot_particles_ZvsTIME(electron_data)
    speed_VS_time(electron_data)
    KE_VS_time(electron_data)
    KE_VS_Z(electron_data)
    plot_timesteps(electron_data)
    timesteps_vs_energy(electron_data)
    
    
    time_checks, num_electrons = num_electrons_per_T(electron_data, 100.0)
    ave_kev_per_T(time_checks, num_electrons, electron_data)
