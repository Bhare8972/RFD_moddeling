...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_float;
	...
	if( n > 0 ) gsl_block_float_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_float_free() to deallocate the memory.
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_float_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_float().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_float to move.
     */
    block_float( block_float&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    block_float& operator=( block_float&& v ){
      block_float( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_float not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    float* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_float", __FILE__, __LINE__, GSL_EFAULT );
    ...
    float& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_float is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_float", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
