...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<short> or similar object as if it were a gsl::vector_short object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_short object; so it
   * may be safer to create gsl::vector_short objects in this way as needed.
   */
  class vector_short {
  public:
    /**
    ...
    explicit vector_short( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_short_alloc( n );
      else { ccgsl_pointer = new gsl_vector_short; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_short but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_short_free( ccgsl_pointer );
    ...
    vector_short( vector_short const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_short is now shared.
    }
    /**
     * The copy constructor. This shares the vector_short. Use clone() if you want a full copy.
     * @param v The vector_short to copy.
     */
    vector_short( vector_short& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_short is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_short_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<short> or std::array<short,size_t> object.
     ...
     * the size of the gsl::vector_short does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_short( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_short*>( malloc( sizeof( gsl_vector_short ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_short without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_short without the possibility deleting it when @c this
     ...
     * gsl::vector_short* argument to one that takes a gsl_vector_short* argument.
     * @param v The gsl_vector_short
     */
     ...
     */
    void reset(){ vector_short().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_short to move.
     */
    vector_short( vector_short&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_short& operator=( vector_short&& v ){
      vector_short( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_short stride is not 1. However, new vector_short objects are always initialised
	...
	gsl_error( "vector_short does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    short& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_short is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_short", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_short* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_short.
     * @return the gsl_vector_short
     */
    gsl_vector_short const* get() const { return ccgsl_pointer; }
    /**
    ...
    /**
     * Another C++ version of gsl_vector_short_const_view_array().
     * @param v An array of type short
     * @param n The size of the vector_short
     * @return A vector_short
     */
    static vector_short const view_array( short const* v, size_t n ){
      gsl_vector_short* w = static_cast<gsl_vector_short*>( malloc( sizeof( gsl_vector_short ) ) );
      *w = gsl_vector_short_const_view_array( v, n ).vector;
      return vector_short( w );
    }
    /**
     * Another C++ version of gsl_vector_short_const_view_array_with_stride().
     * @param base An array of type short
     * @param stride The stride
     * @param n The size of the vector_short
     * @return A vector_short
     */
    static vector_short const view_array_with_stride( short const* base, size_t stride, size_t n ){
      gsl_vector_short* w = static_cast<gsl_vector_short*>( malloc( sizeof( gsl_vector_short ) ) );
      *w = gsl_vector_short_const_view_array_with_stride( base, stride, n ).vector;
      return vector_short( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_short_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_short const subvector( size_t i, size_t n ) const {
      gsl_vector_short* w = static_cast<gsl_vector_short*>( malloc( sizeof( gsl_vector_short ) ) );
      *w = gsl_vector_short_const_subvector( get(), i, n ).vector;
      return vector_short( w );
    }
    /**
     * Another C++ version of gsl_vector_short_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_short const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_short* w = static_cast<gsl_vector_short*>( malloc( sizeof( gsl_vector_short ) ) );
      *w = gsl_vector_short_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_short( w );
    }
