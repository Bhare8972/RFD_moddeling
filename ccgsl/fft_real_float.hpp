/*
 * $Id: fft_real_float.hpp 283 2012-08-26 15:36:43Z jdl3 $
...
#ifndef CCGSL_FFT_REAL_FLOAT_HPP
#define CCGSL_FFT_REAL_FLOAT_HPP
...
namespace gsl {
  namespace fft {
	 ...
	 */
	inline int transform( float data[], size_t const stride, size_t const n ){
	  return gsl_fft_real_float_radix2_transform( data, stride, n ); }

	/**
	 * C++ version of gsl_fft_complex_radix2_transform().
	 * Works with std::array<float>, std::vector<float> and gsl::vector_float.
	 * @param data An array of complex values packed as an array of double length of float.
	 * @param stride The stride of the array.
	 * @return Error code on failure.
	 */
	...
	inline int transform( float data[], size_t const n ){
      ...
      /**
       * C++ version of gsl_fft_real_wavetable_float functions.
       */
      class wavetable {
      public:
	/**
	...
	/**
	 * Could construct from a gsl_fft_real_wavetable_float.
	 * This is not usually a good idea. In this case
	 * you should not use gsl_fft_real_wavetable_float() to deallocate the memory.
	 * @param v The wavetable_float
	 */
	explicit wavetable( gsl_fft_real_wavetable_float* v ){
	  ccgsl_pointer = v;
	 ...
	 * @param v The wavetable_float to copy.
	 */
	 ...
	 * The shared pointer
	 */
	gsl_fft_real_wavetable_float* ccgsl_pointer;
	/**
	...
	/**
	 * Get the gsl_fft_real_wavetable_float.
	 * @return the gsl_fft_real_wavetable_float
	 */
	gsl_fft_real_wavetable_float* get() const { return ccgsl_pointer; }
	/**
	 * Find if this is the only object sharing the gsl_fft_real_wavetable_float.
	 * @return @c true or @c falses according as 
	 * this is the only workspace object sharing the gsl_fft_real_wavetable_float
	 */
	 ...
	 * to a gsl_fft_real_wavetable_float
	 */
#ifdef __GXX_EXPERIMENTAL_CXX0X__
	explicit
#endif
	...
	explicit workspace( size_t const n ){
	  ccgsl_pointer = gsl_fft_real_workspace_float_alloc( n );
	  // just plausibly we could allocate workspace but not count
	  try { count = new size_t; } catch( std::bad_alloc& e ){
	    // try to tidy up before rethrowing
	    gsl_fft_real_workspace_float_free( ccgsl_pointer );
	    throw e;
	  }
	  *count = 1; // initially there is just one reference to ccgsl_pointer
	}
	/**
	 * Could construct from a gsl_fft_real_workspace_float.
	 * This is not usually a good idea. In this case
	 * you should not use gsl_fft_real_workspace_float_free() to deallocate the memory.
	 * @param v The workspace
	 */
	explicit workspace( gsl_fft_real_workspace_float* v ){
	  ccgsl_pointer = v;
	  // just plausibly we could fail to allocate count: no further action needed.
	  count = new size_t;
	  *count = 1; // initially there is just one reference to ccgsl_pointer
	...
	workspace& operator=( workspace const& v ){
	  // first, possibly delete anything pointed to by this
	  if( count == 0 or --*count == 0 ){
	    if( ccgsl_pointer != 0 ) gsl_fft_real_workspace_float_free( ccgsl_pointer );
	    delete count;
	  } // Then copy
	  ccgsl_pointer = v.ccgsl_pointer; count = v.count; if( count != 0 ) ++*count; return *this;
	}
	 ...
	 * The shared pointer
	 */
	gsl_fft_real_workspace_float* ccgsl_pointer;
	/**
	 * The shared reference count
	 */
	size_t* count;
	 ...
	 * @return the gsl_fft_real_workspace_float
	 */
	gsl_fft_real_workspace_float* get() const { return ccgsl_pointer; }
	/**
	 * Find if this is the only object sharing the gsl_fft_real_workspace_float.
	 * @return @c true or @c falses according as 
	 * this is the only workspace object sharing the gsl_fft_real_workspace_float
	 */
	bool unique() const { return count != 0 and *count == 1; }
	/**
	 * Find how many workspace objects share this pointer.
	 ...
	 * Allow conversion to bool.
	 * @return @c true or @c false according as this contains a pointer
	 * to a gsl_fft_real_workspace_float
	 */
#ifdef __GXX_EXPERIMENTAL_CXX0X__
	explicit
#endif
	...
	return gsl_fft_real_float_transform( data, stride, n, wavetable.get(), work.get() ); }

      /**
       * C++ version of gsl_fft_real_float_unpack().
       * @param real_float_coefficient An array of float values.
       * @param complex_coefficient An array of float values for complex coefficients.
       * @param stride The stride of the array.
       * @param n The size of the array.
       * @return Error code on failure.
       */
      inline int unpack( float const real_float_coefficient[], float complex_coefficient[],
			 size_t const stride, size_t const n ){
	return gsl_fft_real_float_unpack( real_float_coefficient, complex_coefficient, stride, n ); }
       ...
       */
      template<typename DATA>
      inline int transform( DATA& data, size_t const stride, wavetable const& wavetable, workspace& work ){
	return gsl_fft_real_float_transform( data.data(), stride, data.size() / stride,
      ...
      inline int unpack( R const& real_float_coefficient, C& complex_coefficient, size_t const stride ){
	size_t n = std::max( real_float_coefficient.size(), complex_coefficient.size() / 2) / stride;
	return gsl_fft_real_float_unpack( real_float_coefficient.data(), complex_coefficient.data(),
      ...
      inline int unpack( vector_float const& real_float_coefficient,
		  vector_complex_float& complex_coefficient, size_t const stride ){
	size_t n = std::max( real_float_coefficient.size(), complex_coefficient.size() ) / stride;
	return gsl_fft_real_float_unpack( real_float_coefficient.data(), complex_coefficient.data(),
					  stride, n ); }
      ...
      inline int transform( float data[], size_t const n,
			    wavetable const& wavetable, workspace& work ){
	return gsl_fft_real_float_transform( data, 1, n, wavetable.get(), work.get() ); }
       ...
       * @return Error code on failure.
       */
      inline int unpack( float const real_float_coefficient[], float complex_coefficient[], size_t const n ){
	return gsl_fft_real_float_unpack( real_float_coefficient, complex_coefficient, 1, n ); }
       ...
       */
      template<typename DATA>
      inline int transform( DATA& data, wavetable const& wavetable, workspace& work ){
	return gsl_fft_real_float_transform( data.data(), 1, data.size(),
				       wavetable.get(), work.get() ); }
       ...
       */
      template<typename R, typename C>
      inline int unpack( R const& real_float_coefficient, C& complex_coefficient ){
	size_t n = std::max( real_float_coefficient.size(), complex_coefficient.size() / 2);
	return gsl_fft_real_float_unpack( real_float_coefficient.data(), complex_coefficient.data(),
				    1, n ); }
      // Partial specialisation
