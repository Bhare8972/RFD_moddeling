/*
 * $Id: block_complex_float.hpp 288 2012-12-04 15:31:18Z jdl3 $
...
namespace gsl {
  ...
  public:
    ...
    explicit block_complex_float( size_t const n ){
      ...
      if( n > 0 )
	ccgsl_pointer = gsl_block_complex_float_alloc( n );
      else {
	ccgsl_pointer = new gsl_block_complex_float;
	...
	if( n > 0 ) gsl_block_complex_float_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_complex_float_free() to deallocate the memory.
	...
	*p = complex_float( std::move( x ) ); ++p;
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_complex_float_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_complex_float().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_complex_float to move.
     */
    block_complex_float( block_complex_float&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    block_complex_float& operator=( block_complex_float&& v ){
      block_complex_float( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
