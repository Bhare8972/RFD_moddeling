/*
 * $Id: block_complex.hpp 288 2012-12-04 15:31:18Z jdl3 $
...
namespace gsl {
  ...
  public:
    ...
    explicit block_complex( size_t const n ){
      ...
      if( n > 0 )
	ccgsl_pointer = gsl_block_complex_alloc( n );
      else {
	ccgsl_pointer = new gsl_block_complex;
	...
	if( n > 0 ) gsl_block_complex_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_complex_free() to deallocate the memory.
	...
	*p = complex( std::move( x ) ); ++p;
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_complex_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_complex().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_complex to move.
     */
    block_complex( block_complex&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    block_complex& operator=( block_complex&& v ){
      block_complex( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
	  ...
	  gsl_error( "block_complex not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return complex_ref( 0 );
  	}
