...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<long> or similar object as if it were a gsl::vector_long object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_long object; so it
   * may be safer to create gsl::vector_long objects in this way as needed.
   */
  class vector_long {
  public:
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_long_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * the size of the gsl::vector_long does not change. Stride is normally set to 1 and allows us
...
*/
    ...
    // Allow possibility of assigning from gsl_vector_long without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_long without the possibility deleting it when @c this
     ...
     * gsl::vector_long* argument to one that takes a gsl_vector_long* argument.
     * @param v The gsl_vector_long
     */
     ...
     */
    void reset(){ vector_long().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_long to move.
     */
    vector_long( vector_long&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_long& operator=( vector_long&& v ){
      vector_long( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_long stride is not 1. However, new vector_long objects are always initialised
	...
	gsl_error( "vector_long does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    long& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_long is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_long", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_long* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_long.
     * @return the gsl_vector_long
     */
    ...
    /**
     * Another C++ version of gsl_vector_long_const_view_array().
     * @param v An array of type long
     * @param n The size of the vector_long
     * @return A vector_long
     */
    static vector_long const view_array( long const* v, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_view_array( v, n ).vector;
      return vector_long( w );
    }
    /**
     * Another C++ version of gsl_vector_long_const_view_array_with_stride().
     * @param base An array of type long
     * @param stride The stride
     * @param n The size of the vector_long
     * @return A vector_long
     */
    static vector_long const view_array_with_stride( long const* base, size_t stride, size_t n ){
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_view_array_with_stride( base, stride, n ).vector;
      return vector_long( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_long_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_long const subvector( size_t i, size_t n ) const {
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_subvector( get(), i, n ).vector;
      return vector_long( w );
    }
    /**
     * Another C++ version of gsl_vector_long_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_long const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_long* w = static_cast<gsl_vector_long*>( malloc( sizeof( gsl_vector_long ) ) );
      *w = gsl_vector_long_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_long( w );
    }
