...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<char> or similar object as if it were a gsl::vector_char object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_char object; so it
   * may be safer to create gsl::vector_char objects in this way as needed.
   */
  class vector_char {
  public:
    /**
    ...
    explicit vector_char( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_char_alloc( n );
      else { ccgsl_pointer = new gsl_vector_char; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_char but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_char_free( ccgsl_pointer );
    ...
    vector_char( vector_char const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_char is now shared.
    }
    /**
     * The copy constructor. This shares the vector_char. Use clone() if you want a full copy.
     * @param v The vector_char to copy.
     */
    vector_char( vector_char& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_char is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_char_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<char> or std::array<char,size_t> object.
     ...
     * the size of the gsl::vector_char does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_char( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_char*>( malloc( sizeof( gsl_vector_char ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_char without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_char without the possibility deleting it when @c this
     ...
     * gsl::vector_char* argument to one that takes a gsl_vector_char* argument.
     * @param v The gsl_vector_char
     */
     ...
     */
    void reset(){ vector_char().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_char to move.
     */
    vector_char( vector_char&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_char& operator=( vector_char&& v ){
      vector_char( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
    ...
    /**
     * Give access to the data block_char. The data() and size() functions mimic the functions
     * of std::array<T> and std::vector<T>. This function can throw an exception or produce
     * a GSL error if the vector_char stride is not 1. However, new vector_char objects are always initialised
     * to have a stride of 1.
     * @return The data block_char.
     */
    char* data() { 
      if( ccgsl_pointer == 0 ) gsl_error( "null vector_char", __FILE__, __LINE__, GSL_EFAULT );
#ifndef GSL_RANGE_CHECK_OFF
      if( ccgsl_pointer->stride != 1 )
	gsl_error( "vector_char does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    char& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_char is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_char", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_char* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_char.
     * @return the gsl_vector_char
     */
    gsl_vector_char const* get() const { return ccgsl_pointer; }
    /**
    ...
    /**
     * Another C++ version of gsl_vector_char_const_view_array().
     * @param v An array of type char
     * @param n The size of the vector_char
     * @return A vector_char
     */
    static vector_char const view_array( char const* v, size_t n ){
      gsl_vector_char* w = static_cast<gsl_vector_char*>( malloc( sizeof( gsl_vector_char ) ) );
      *w = gsl_vector_char_const_view_array( v, n ).vector;
      return vector_char( w );
    }
    /**
     * Another C++ version of gsl_vector_char_const_view_array_with_stride().
     * @param base An array of type char
     * @param stride The stride
     * @param n The size of the vector_char
     * @return A vector_char
     */
    static vector_char const view_array_with_stride( char const* base, size_t stride, size_t n ){
      gsl_vector_char* w = static_cast<gsl_vector_char*>( malloc( sizeof( gsl_vector_char ) ) );
      *w = gsl_vector_char_const_view_array_with_stride( base, stride, n ).vector;
      return vector_char( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_char_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_char const subvector( size_t i, size_t n ) const {
      gsl_vector_char* w = static_cast<gsl_vector_char*>( malloc( sizeof( gsl_vector_char ) ) );
      *w = gsl_vector_char_const_subvector( get(), i, n ).vector;
      return vector_char( w );
    }
    /**
     * Another C++ version of gsl_vector_char_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_char const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_char* w = static_cast<gsl_vector_char*>( malloc( sizeof( gsl_vector_char ) ) );
      *w = gsl_vector_char_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_char( w );
    }
