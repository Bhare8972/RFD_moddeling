...
namespace gsl {
  ...
  namespace monte {
	 ...
	 * we should not use gsl_monte_miser_free() to deallocate the memory.
	 * @param v The state
	 */
	 ...
	 * @return the gsl_monte_miser_state
	 */
	 ...
	 * this is the only state object sharing the gsl_monte_miser_state.
	 */
	 ...
	 * @return min_calls_per_bisection
	 */
	...
	/**
	 * Set dither.
	 * @param dither New value for dither
	 */
	void set_dither( double const dither ){ ccgsl_pointer->dither = dither; }
      };
      /**
       * C++ version of gsl_monte_miser_init(). This allows this state to be reused.
	...
	return gsl_monte_miser_integrate( f, xl, xu, dim, calls, r.get(), state.get(),
					  result, abserr ); } 
      /**
       * C++ version of gsl_monte_miser_integrate(). This version can use gsl::vector
