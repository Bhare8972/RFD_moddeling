...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_long_double;
	...
	if( n > 0 ) gsl_block_long_double_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_long_double_free() to deallocate the memory.
     ...
     */
    block_long_double( std::initializer_list<long double> initializer_list ){
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_long_double_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_long_double().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_long_double to move.
     */
    block_long_double( block_long_double&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_long_double& operator=( block_long_double&& v ){
      block_long_double( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_long_double not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    long double* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_long_double", __FILE__, __LINE__, GSL_EFAULT );
    ...
    long double& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_long_double is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_long_double", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
