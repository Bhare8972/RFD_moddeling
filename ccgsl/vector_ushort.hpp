...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<unsigned short> or similar object as if it were a gsl::vector_ushort object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_ushort object; so it
   * may be safer to create gsl::vector_ushort objects in this way as needed.
   */
  class vector_ushort {
  public:
    //my additions
    vector_ushort vec_and(vector_ushort const& v)
    {
		...
		{
			out[i]=((*this)[i]!=0) and (v[i]!=0);
		}
		return out;
	}

    vector_ushort vec_or(vector_ushort const& v)
    {
		// check if vectors are null
		if( ccgsl_pointer == 0 ) throw vector_exception();
		if( v.ccgsl_pointer == 0 ) throw vector_exception();
    ...
    explicit vector_ushort( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_ushort_alloc( n );
      else { ccgsl_pointer = new gsl_vector_ushort; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_ushort but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_ushort_free( ccgsl_pointer );
    ...
    vector_ushort( vector_ushort const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_ushort is now shared.
    }
    /**
     * The copy constructor. This shares the vector_ushort. Use clone() if you want a full copy.
     * @param v The vector_ushort to copy.
     */
    vector_ushort( vector_ushort& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_ushort is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_ushort_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<unsigned short> or std::array<unsigned short,size_t> object.
     ...
     * the size of the gsl::vector_ushort does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_ushort( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_ushort without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_ushort without the possibility deleting it when @c this
     ...
     * gsl::vector_ushort* argument to one that takes a gsl_vector_ushort* argument.
     * @param v The gsl_vector_ushort
     */
     ...
     */
    void reset(){ vector_ushort().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_ushort to move.
     */
    vector_ushort( vector_ushort&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    vector_ushort& operator=( vector_ushort&& v ){
      vector_ushort( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_ushort stride is not 1. However, new vector_ushort objects are always initialised
	...
	gsl_error( "vector_ushort does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    unsigned short& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_ushort is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_ushort", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_ushort* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_ushort.
     * @return the gsl_vector_ushort
     */
    gsl_vector_ushort const* get() const { return ccgsl_pointer; }
    /**
    ...
    /**
     * Another C++ version of gsl_vector_ushort_const_view_array().
     * @param v An array of type unsigned short
     * @param n The size of the vector_ushort
     * @return A vector_ushort
     */
    static vector_ushort const view_array( unsigned short const* v, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_view_array( v, n ).vector;
      return vector_ushort( w );
    }
    /**
     * Another C++ version of gsl_vector_ushort_const_view_array_with_stride().
     * @param base An array of type unsigned short
     * @param stride The stride
     * @param n The size of the vector_ushort
     * @return A vector_ushort
     */
    static vector_ushort const view_array_with_stride( unsigned short const* base, size_t stride, size_t n ){
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_view_array_with_stride( base, stride, n ).vector;
      return vector_ushort( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_ushort_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_ushort const subvector( size_t i, size_t n ) const {
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_subvector( get(), i, n ).vector;
      return vector_ushort( w );
    }
    /**
     * Another C++ version of gsl_vector_ushort_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_ushort const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_ushort* w = static_cast<gsl_vector_ushort*>( malloc( sizeof( gsl_vector_ushort ) ) );
      *w = gsl_vector_ushort_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_ushort( w );
    }
