...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_char;
	...
	if( n > 0 ) gsl_block_char_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_char_free() to deallocate the memory.
     ...
     */
    block_char( std::initializer_list<char> initializer_list ){
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_char_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_char().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_char to move.
     */
    block_char( block_char&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_char& operator=( block_char&& v ){
      block_char( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_char not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    char* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_char", __FILE__, __LINE__, GSL_EFAULT );
    ...
    char& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_char is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_char", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
