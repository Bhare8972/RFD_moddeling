...
namespace gsl {
   ...
   * Changing the size of the original object does not change the size of the gsl::vector_uchar object; so it
   * may be safer to create gsl::vector_uchar objects in this way as needed.
   */
  class vector_uchar {
  public:
    /**
    ...
    explicit vector_uchar( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_uchar_alloc( n );
      else { ccgsl_pointer = new gsl_vector_uchar; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_uchar but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_uchar_free( ccgsl_pointer );
    ...
    vector_uchar( vector_uchar const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_uchar is now shared.
    }
    /**
     * The copy constructor. This shares the vector_uchar. Use clone() if you want a full copy.
     * @param v The vector_uchar to copy.
     */
    vector_uchar( vector_uchar& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_uchar is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_uchar_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<unsigned char> or std::array<unsigned char,size_t> object.
     ...
     * the size of the gsl::vector_uchar does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_uchar( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_uchar*>( malloc( sizeof( gsl_vector_uchar ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_uchar without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_uchar without the possibility deleting it when @c this
     ...
     * gsl::vector_uchar* argument to one that takes a gsl_vector_uchar* argument.
     * @param v The gsl_vector_uchar
     */
     ...
     */
    void reset(){ vector_uchar().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_uchar to move.
     */
    vector_uchar( vector_uchar&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_uchar& operator=( vector_uchar&& v ){
      vector_uchar( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_uchar stride is not 1. However, new vector_uchar objects are always initialised
	...
	gsl_error( "vector_uchar does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    unsigned char& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_uchar is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_uchar", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_uchar* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_uchar.
     * @return the gsl_vector_uchar
     */
    gsl_vector_uchar const* get() const { return ccgsl_pointer; }
    /**
    ...
    /**
     * Another C++ version of gsl_vector_uchar_const_view_array().
     * @param v An array of type unsigned char
     * @param n The size of the vector_uchar
     * @return A vector_uchar
     */
    static vector_uchar const view_array( unsigned char const* v, size_t n ){
      gsl_vector_uchar* w = static_cast<gsl_vector_uchar*>( malloc( sizeof( gsl_vector_uchar ) ) );
      *w = gsl_vector_uchar_const_view_array( v, n ).vector;
      return vector_uchar( w );
    }
    /**
     * Another C++ version of gsl_vector_uchar_const_view_array_with_stride().
     * @param base An array of type unsigned char
     * @param stride The stride
     * @param n The size of the vector_uchar
     * @return A vector_uchar
     */
    static vector_uchar const view_array_with_stride( unsigned char const* base, size_t stride, size_t n ){
      gsl_vector_uchar* w = static_cast<gsl_vector_uchar*>( malloc( sizeof( gsl_vector_uchar ) ) );
      *w = gsl_vector_uchar_const_view_array_with_stride( base, stride, n ).vector;
      return vector_uchar( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_uchar_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_uchar const subvector( size_t i, size_t n ) const {
      gsl_vector_uchar* w = static_cast<gsl_vector_uchar*>( malloc( sizeof( gsl_vector_uchar ) ) );
      *w = gsl_vector_uchar_const_subvector( get(), i, n ).vector;
      return vector_uchar( w );
    }
    /**
     * Another C++ version of gsl_vector_uchar_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_uchar const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_uchar* w = static_cast<gsl_vector_uchar*>( malloc( sizeof( gsl_vector_uchar ) ) );
      *w = gsl_vector_uchar_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_uchar( w );
    }
