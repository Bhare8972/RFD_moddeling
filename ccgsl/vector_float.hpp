...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<float> or similar object as if it were a gsl::vector_float object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_float object; so it
   * may be safer to create gsl::vector_float objects in this way as needed.
   */
  class vector_float {
  public:
    ...
    vector_float operator+(vector_float const& v) const
    {
		...
		//ADD!
		vector_float new_vector(ccgsl_pointer->size);
		for( size_t i = 0; i<ccgsl_pointer->size; ++i )
		{
    ...
    void operator+=(vector_float const& v)
    {
		// check if vectors are null
		if( ccgsl_pointer == 0 ) throw vector_exception();
		if( v.ccgsl_pointer == 0 ) throw vector_exception();
	...
	friend vector_float operator*(double const& lhs, vector_float const& rhs)
    ...
    vector_ushort vec_lessThan(vector_float const& v)
    {
		// check if vectors are null
		if( ccgsl_pointer == 0 ) throw vector_exception();
		if( v.ccgsl_pointer == 0 ) throw vector_exception();
    ...
    vector_ushort vec_greaterThan(vector_float const& v)
    {
		// check if vectors are null
		if( ccgsl_pointer == 0 ) throw vector_exception();
		if( v.ccgsl_pointer == 0 ) throw vector_exception();
    ...
    vector_ushort vec_equalTo(vector_float const& v)
    {
		// check if vectors are null
		if( ccgsl_pointer == 0 ) throw vector_exception();
		if( v.ccgsl_pointer == 0 ) throw vector_exception();
    ...
    explicit vector_float( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_float_alloc( n );
      else { ccgsl_pointer = new gsl_vector_float; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_float_free( ccgsl_pointer );
    ...
    vector_float( vector_float const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_float is now shared.
    }
    /**
     * The copy constructor. This shares the vector_float. Use clone() if you want a full copy.
     * @param v The vector_float to copy.
     */
    vector_float( vector_float& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_float is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_float_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<float> or std::array<float,size_t> object.
     ...
     * the size of the gsl::vector_float does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_float( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_float*>( malloc( sizeof( gsl_vector_float ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_float without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_float without the possibility deleting it when @c this
     ...
     * gsl::vector_float* argument to one that takes a gsl_vector_float* argument.
     * @param v The gsl_vector_float
     */
     ...
     */
    void reset(){ vector_float().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_float to move.
     */
    vector_float( vector_float&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_float& operator=( vector_float&& v ){
      vector_float( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_float stride is not 1. However, new vector_float objects are always initialised
	...
	gsl_error( "vector_float does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    float& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_float is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_float", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_float* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_float.
     * @return the gsl_vector_float
     */
    ...
    /**
     * Another C++ version of gsl_vector_float_const_view_array().
     * @param v An array of type float
     * @param n The size of the vector_float
     * @return A vector_float
     */
    static vector_float const view_array( float const* v, size_t n ){
      gsl_vector_float* w = static_cast<gsl_vector_float*>( malloc( sizeof( gsl_vector_float ) ) );
      *w = gsl_vector_float_const_view_array( v, n ).vector;
      return vector_float( w );
    }
    /**
     * Another C++ version of gsl_vector_float_const_view_array_with_stride().
     * @param base An array of type float
     * @param stride The stride
     * @param n The size of the vector_float
     * @return A vector_float
     */
    static vector_float const view_array_with_stride( float const* base, size_t stride, size_t n ){
      gsl_vector_float* w = static_cast<gsl_vector_float*>( malloc( sizeof( gsl_vector_float ) ) );
      *w = gsl_vector_float_const_view_array_with_stride( base, stride, n ).vector;
      return vector_float( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_float_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_float const subvector( size_t i, size_t n ) const {
      gsl_vector_float* w = static_cast<gsl_vector_float*>( malloc( sizeof( gsl_vector_float ) ) );
      *w = gsl_vector_float_const_subvector( get(), i, n ).vector;
      return vector_float( w );
    }
    /**
     * Another C++ version of gsl_vector_float_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_float const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_float* w = static_cast<gsl_vector_float*>( malloc( sizeof( gsl_vector_float ) ) );
      *w = gsl_vector_float_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_float( w );
    }
