...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_short;
	...
	if( n > 0 ) gsl_block_short_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_short_free() to deallocate the memory.
     ...
     */
    block_short( std::initializer_list<short> initializer_list ){
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_short_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_short().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_short to move.
     */
    block_short( block_short&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_short& operator=( block_short&& v ){
      block_short( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_short not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    short* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_short", __FILE__, __LINE__, GSL_EFAULT );
    ...
    short& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_short is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_short", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
