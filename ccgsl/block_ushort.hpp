...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_ushort;
	...
	if( n > 0 ) gsl_block_ushort_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_ushort_free() to deallocate the memory.
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_ushort_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_ushort().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_ushort to move.
     */
    block_ushort( block_ushort&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_ushort& operator=( block_ushort&& v ){
      block_ushort( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_ushort not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    unsigned short* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_ushort", __FILE__, __LINE__, GSL_EFAULT );
    ...
    unsigned short& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_ushort is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_ushort", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
