...
namespace gsl {
   ...
   * Otherwise,
   * @code
   * for( matrix_float::iterator i = matrix_float.begin(); i != matrix_float.end(); ++i ){
   *   vector_float row = *i;
   *   ...
   * }
   * @endcode
   ...
   * i-> ...
   * @endcode
   * because each call of @c operator->() creates a new and different vector_float.
   */
  class matrix_float {
  public:
    /**
    ...
    explicit matrix_float( size_t const n1, size_t const n2 ){
      if( n1 > 0 and n2 > 0 ) ccgsl_pointer = gsl_matrix_float_alloc( n1, n2 );
      else { ccgsl_pointer = new gsl_matrix_float; ccgsl_pointer->size1 = n1;
	ccgsl_pointer->size2 = n2; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate matrix_float but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n1 > 0 and n2 > 0 ) gsl_matrix_float_free( ccgsl_pointer );
     ...
     * you should not use gsl_matrix_float_free() to deallocate the memory.
	...
	if( l.size() != n2 ){
	  gsl::exception e( "matrix_float rows have unequal sizes", __FILE__, __LINE__,
    ...
    matrix_float( matrix_float const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // matrix_float is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 and ccgsl_pointer->size1 > 0 and ccgsl_pointer->size2 > 0 ) gsl_matrix_float_free( ccgsl_pointer );
	else delete ccgsl_pointer;
	delete count;
      }
      // Then copy
      ...
      count = v.count;
      if( count != 0 ) ++*count; // block_float is now shared.
      return *this;
    }
    // clone()
    /**
    ...
    // Allow possibility of assigning from gsl_matrix_float without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_matrix_float without the possibility deleting it when @c this
     ...
     * gsl::matrix_float* argument to one that takes a gsl_matrix_float* argument.
     * @param v The gsl_matrix_float
     */
     ...
     */
    void reset(){ matrix_float().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The matrix_float to move.
     */
    matrix_float( matrix_float&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    matrix_float& operator=( matrix_float&& v ){
      matrix_float( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  private:
     ...
     */
    struct vector_float_ptr : public vector_float {
      /**
       * Typically we have to construct from a vector_float.
       * @param v The vector_float to construct from.
       */
      vector_float_ptr( vector_float const& v ) : vector_float( v ){}
      /**
       * Dereference operator.
       * @return a vector_float
       */
      vector_float& operator*(){ return *this; }
      /**
       * Dereference operator.
       * @return a pointer to a vector_float
       */
      vector_float* operator->(){ return this; }
    };
    /**
    ...
    public:
       ...
       */
      typedef vector_float value_type;
      /**
       * An iterator must have a pointer typea.
       */
      typedef vector_float_ptr pointer;
      /**
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "matrix_float not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	}
	...
	static vector_float_ptr something( something_base );
  	// First check that iterator is initialised.
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return something;
  	...
  	// position and v are valid: return data
  	vector_float_ptr ptr( v->row( position ) );
#ifdef __GXX_EXPERIMENTAL_CXX0X__
  	return std::move( ptr );
#else
  	return ptr;
    ...
    protected:
       ...
       */
      void increment(){
  	// Only makes sense if v points to a matrix_float
  	if( v == 0 ){
  	  gsl_error( "iterator not initialised", __FILE__, __LINE__, exception::GSL_EFAILED );
  	  return;
  	} else if( v->ccgsl_pointer == 0 ){
       ...
       * @param v The matrix_float that creates @c this
      ...
      /**
       * Store a pointer to a matrix_float we can iterate over: 0 if no matrix_float.
       */
      container* v;
      /**
       * Mark position of iterator within matrix_float.
       */
      size_t position;
    };
     ...
     */
    template<bool reverse_t> class iterator_t : public iterator_base<matrix_float,vector_float,reverse_t>{
    public:
  	...
  	iterator_base<matrix_float,vector_float,reverse_t>::v = i.v;
  	iterator_base<matrix_float,vector_float,reverse_t>::position = i.position;
  	return *this;
      }
  	...
  	iterator_base<matrix_float,vector_float,reverse_t>::increment();
  	return *this;
      }
      /**
       ...
       */
      iterator_t<reverse_t>& operator--(){
  	iterator_base<matrix_float,vector_float,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       */
      iterator_t<reverse_t>() : iterator_base<matrix_float,vector_float,reverse_t>(){}
    protected:
      friend class matrix_float;
      // We need a constructor for matrix_float
      /**
       * This constructor allows vector_float to create non-default iterators.
       * @param v The matrix_float that creates @c this
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( matrix_float* v, size_t position )
      : iterator_base<matrix_float,vector_float,reverse_t>( v, position ){}
    };
    /**
     * A class template for the const iterators.
     */
    template<bool reverse_t> class const_iterator_t
      : public iterator_base<matrix_float const,vector_float const,reverse_t>{
    public:
      // // Refines output iterator
      // // operator=
      /**
  	...
  	iterator_base<matrix_float const,vector_float const,reverse_t>::v = i.v;
  	iterator_base<matrix_float const,vector_float const,reverse_t>::position = i.position;
  	return *this;
      }
      // // Refines forward iterator
      // // operator++ (both)
  	...
  	iterator_base<matrix_float const,vector_float const,reverse_t>::increment();
  	return *this;
      }
      /**
       * The postfix ++ operator.
       ...
       */
      const_iterator_t<reverse_t>& operator--(){
  	iterator_base<matrix_float const,vector_float const,reverse_t>::decrement();
  	return *this;
      }
      /**
       * The postfix -- operator.
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<matrix_float,vector_float,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( matrix_float const* v, size_t position )
      : iterator_base<matrix_float const,vector_float const,reverse_t>( v, position ){}
    };
  public:
    /**
  ...
  public:
    ...
    /**
     * Give access to the data block_float. The data() and size() functions mimic the functions
     * of std::array<T> and std::vector<T>. This function can throw an exception or produce
     * a GSL error if the matrix_float size2 and tda are not equal.
     * However, new matrix_float objects are always initialised to have tda and size2 equal.
	...
	gsl_error( "matrix_float size2 and tda do not match", __FILE__, __LINE__, GSL_EBADLEN );
#endif
      return ccgsl_pointer->data; }
    /**
     * Give access to the data block_float. The data() and size() functions mimic the functions
     * of std::array<T> and std::vector<T>. This function can throw an exception or produce
     * a GSL error if the matrix_float size1 and tda are not equal.
     * However, new matrix_float objects are always initialised to have tda and size2 equal.
     * @return The data block_float.
     */
     ...
     */
    static matrix_float const
    const_view_vector_with_tda( vector_float const& v, size_t const n1, size_t const n2, size_t const tda ){
      gsl_matrix_float* m = static_cast<gsl_matrix_float*>( malloc( sizeof( gsl_matrix_float ) ) );
     ...
     */
    static matrix_float const
    view_vector_with_tda( vector_float const& v, size_t const n1, size_t const n2, size_t const tda ){
      gsl_matrix_float* m = static_cast<gsl_matrix_float*>( malloc( sizeof( gsl_matrix_float ) ) );
      *m = gsl_matrix_float_const_view_vector_with_tda( v.get(), n1, n2, tda ).matrix;
      return matrix_float( m );
    }
  ...
  public:
     ...
     */
    gsl_matrix_float* get(){ return ccgsl_pointer; }
    /**
     * Get the gsl_matrix_float.
     * @return the gsl_matrix_float
     */
    gsl_matrix_float const* get() const { return ccgsl_pointer; }
    /**
     ...
     * Although @c matrix_float[i][j] is possible, it is less efficient than
    ...
    vector_float operator[]( size_t const i ){
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "matrix_float is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return vector_float();
      }
#ifndef GSL_RANGE_CHECK_OFF
#ifndef __GXX_EXPERIMENTAL_CXX0X__
      static vector_float something;
#endif
      //Check that position make sense
      if( i >= size1() ){
  	gsl_error( "trying to read beyond last row of matrix_float",
		   __FILE__, __LINE__, exception::GSL_EINVAL );
#ifdef __GXX_EXPERIMENTAL_CXX0X__
	return std::move( vector_float() );
#else
	return something;
#endif
      }
  ...
  inline vector_float vector_float::alloc_row_from_matrix( matrix_float& m, size_t const i ){
    return vector_float ( gsl_vector_float_alloc_row_from_matrix( m.get(), i ) ); }
  inline vector_float vector_float::alloc_col_from_matrix( matrix_float& m, size_t const i ){
