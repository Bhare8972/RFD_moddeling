...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_uint;
	...
	if( n > 0 ) gsl_block_uint_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_uint_free() to deallocate the memory.
     ...
     */
    block_uint( std::initializer_list<unsigned int> initializer_list ){
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_uint_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_uint().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_uint to move.
     */
    block_uint( block_uint&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_uint& operator=( block_uint&& v ){
      block_uint( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_uint not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    unsigned int* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_uint", __FILE__, __LINE__, GSL_EFAULT );
    ...
    unsigned int& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_uint is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_uint", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
