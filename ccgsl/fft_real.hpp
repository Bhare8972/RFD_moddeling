/*
 * $Id: fft_real.hpp 283 2012-08-26 15:36:43Z jdl3 $
...
#ifndef CCGSL_FFT_REAL_HPP
#define CCGSL_FFT_REAL_HPP
...
namespace gsl {
  namespace fft {
	 ...
	 */
	inline int transform( double data[], size_t const stride, size_t const n ){
	  return gsl_fft_real_radix2_transform( data, stride, n ); }

	/**
	 * C++ version of gsl_fft_complex_radix2_transform().
	 * Works with std::array<double>, std::vector<double> and gsl::vector.
	 * @param data An array of complex values packed as an array of double length of double.
	 * @param stride The stride of the array.
	 * @return Error code on failure.
	 */
	template<typename DATA>
	inline int transform( DATA& data, size_t const stride = 1 ){
	  return gsl_fft_real_radix2_transform( data.data(), stride, data.size() / stride ); }
	 ...
	 * @return Error code on failure.
	 */
	inline int transform( double data[], size_t const n ){
	  return gsl_fft_real_radix2_transform( data, 1, n ); }
      }
	...
	explicit wavetable( size_t const n ){
	  ccgsl_pointer = gsl_fft_real_wavetable_alloc( n );
	  // just plausibly we could allocate wavetable but not count
	  try { count = new size_t; } catch( std::bad_alloc& e ){
	    // try to tidy up before rethrowing
	    gsl_fft_real_wavetable_free( ccgsl_pointer );
	    throw e;
	  }
	 ...
	 * you should not use gsl_fft_real_wavetable() to deallocate the memory.
	 * @param v The wavetable
	 */
	explicit wavetable( gsl_fft_real_wavetable* v ){
	  ccgsl_pointer = v;
	...
	wavetable& operator=( wavetable const& v ){
	  ...
	  if( count == 0 or --*count == 0 ){
	    if( ccgsl_pointer != 0 ) gsl_fft_real_wavetable_free( ccgsl_pointer );
	    delete count;
	  } // Then copy
	 ...
	 * @return the gsl_fft_real_wavetable
	 */
	gsl_fft_real_wavetable* get() const { return ccgsl_pointer; }
	/**
	 * Find if this is the only object sharing the gsl_fft_real_wavetable.
	 * @return @c true or @c falses according as 
	 * this is the only workspace object sharing the gsl_fft_real_wavetable
	 */
	...
	explicit workspace( size_t const n ){
	  ccgsl_pointer = gsl_fft_real_workspace_alloc( n );
	  // just plausibly we could allocate workspace but not count
	  try { count = new size_t; } catch( std::bad_alloc& e ){
	    // try to tidy up before rethrowing
	    gsl_fft_real_workspace_free( ccgsl_pointer );
	    throw e;
	  }
	  *count = 1; // initially there is just one reference to ccgsl_pointer
	}
	 ...
	 * This is not usually a good idea. In this case
	 * you should not use gsl_fft_real_workspace_free() to deallocate the memory.
	 * @param v The workspace
	 */
	explicit workspace( gsl_fft_real_workspace* v ){
	  ccgsl_pointer = v;
	  // just plausibly we could fail to allocate count: no further action needed.
	  count = new size_t;
	  *count = 1; // initially there is just one reference to ccgsl_pointer
	...
	workspace& operator=( workspace const& v ){
	  // first, possibly delete anything pointed to by this
	  if( count == 0 or --*count == 0 ){
	    if( ccgsl_pointer != 0 ) gsl_fft_real_workspace_free( ccgsl_pointer );
	    delete count;
	  } // Then copy
	  ccgsl_pointer = v.ccgsl_pointer; count = v.count; if( count != 0 ) ++*count; return *this;
	}
	 ...
	 * @return the gsl_fft_real_workspace
	 */
	gsl_fft_real_workspace* get() const { return ccgsl_pointer; }
	/**
	 * Find if this is the only object sharing the gsl_fft_real_workspace.
	 * @return @c true or @c falses according as 
	 * this is the only workspace object sharing the gsl_fft_real_workspace
	 */
	bool unique() const { return count != 0 and *count == 1; }
	/**
	 * Find how many workspace objects share this pointer.
	...
	return gsl_fft_real_transform( data, stride, n, wavetable.get(), work.get() ); }
	...
	return gsl_fft_real_unpack( real_coefficient, complex_coefficient, stride, n ); }
	...
	return gsl_fft_real_transform( data.data(), stride, data.size() / stride,
	...
	size_t n = std::max( real_coefficient.size(), complex_coefficient.size() / 2) / stride;
	return gsl_fft_real_unpack( real_coefficient.data(), complex_coefficient.data(),
				    stride, n ); }
       ...
       */
      template<>
      inline int unpack( vector const& real_coefficient,
		  vector_complex& complex_coefficient, size_t const stride ){
	size_t n = std::max( real_coefficient.size(), complex_coefficient.size() ) / stride;
	return gsl_fft_real_unpack( real_coefficient.data(), complex_coefficient.data(),
					  stride, n ); }
      ...
      inline int transform( double data[], size_t const n,
			    wavetable const& wavetable, workspace& work ){
	return gsl_fft_real_transform( data, 1, n, wavetable.get(), work.get() ); }
	...
	return gsl_fft_real_unpack( real_coefficient, complex_coefficient, 1, n ); }
	...
	return gsl_fft_real_transform( data.data(), 1, data.size(),
				       wavetable.get(), work.get() ); }
