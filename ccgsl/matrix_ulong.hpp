...
namespace gsl {
   ...
   *
   * Note that in C++11 it is possible to iterate over the rows of a matrix_ulong using
   * @code
   * for( auto row : matrix_ulong ){ ... }
   * @endcode
   * Otherwise,
   * @code
   * for( matrix_ulong::iterator i = matrix_ulong.begin(); i != matrix_ulong.end(); ++i ){
   *   vector_ulong row = *i;
   *   ...
   * }
   * @endcode
   ...
   * i-> ...
   * @endcode
   * because each call of @c operator->() creates a new and different vector_ulong.
   */
  class matrix_ulong {
  public:
    /**
     ...
     * you should not use gsl_matrix_ulong_free() to deallocate the memory.
     ...
     * to point to a gsl_matrix_ulong without the possibility deleting it when @c this
     ...
     * gsl::matrix_ulong* argument to one that takes a gsl_matrix_ulong* argument.
     * @param v The gsl_matrix_ulong
     */
    ...
    /**
     * Move constructor.
     * @param v The matrix_ulong to move.
     */
    matrix_ulong( matrix_ulong&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
  ...
  private:
    ...
    struct vector_ulong_ptr : public vector_ulong {
       ...
       * @param v The vector_ulong to construct from.
       */
      ...
      /**
       * Dereference operator.
       * @return a pointer to a vector_ulong
       */
      vector_ulong* operator->(){ return this; }
    };
    /**
    ...
    protected:
       ...
       * @param v The matrix_ulong that creates @c this
    ...
    public:
       ...
       */
      iterator_t<reverse_t>() : iterator_base<matrix_ulong,vector_ulong,reverse_t>(){}
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      iterator_t<reverse_t>( matrix_ulong* v, size_t position )
    ...
    public:
       ...
       * The default constructor.
       */
      const_iterator_t<reverse_t>() : iterator_base<matrix_ulong,vector_ulong,reverse_t>(){}
      /**
       * A copy constructor
    ...
    protected:
       ...
       * @param position The initial postion of the iterator
       */
      const_iterator_t<reverse_t>( matrix_ulong const* v, size_t position )
  ...
  public:
     ...
     * a GSL error if the matrix_ulong size2 and tda are not equal.
     * However, new matrix_ulong objects are always initialised to have tda and size2 equal.
     ...
     * Give access to the data block_ulong. The data() and size() functions mimic the functions
     * of std::array<T> and std::vector<T>. This function can throw an exception or produce
     * a GSL error if the matrix_ulong size1 and tda are not equal.
     * However, new matrix_ulong objects are always initialised to have tda and size2 equal.
     * @return The data block_ulong.
     */
  ...
  public:
     ...
     * Although @c matrix_ulong[i][j] is possible, it is less efficient than
