...
namespace gsl {
  ...
  public:
	...
	ccgsl_pointer = new gsl_block_long;
	...
	if( n > 0 ) gsl_block_long_free( ccgsl_pointer );
     ...
     * you should not use gsl_block_long_free() to deallocate the memory.
     ...
     */
    block_long( std::initializer_list<long> initializer_list ){
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_block_long_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     */
    void reset(){ block_long().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The block_long to move.
     */
    block_long( block_long&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
     ...
     */
    block_long& operator=( block_long&& v ){
      block_long( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
  ...
  private:
    ...
    public:
  	...
  	if( v == 0 ){
  	  ...
  	  gsl_error( "block_long not initialised", __FILE__, __LINE__, exception::GSL_EFAULT );
  	  return something;
  	}
  ...
  public:
     ...
     */
    long* data() {
      if( ccgsl_pointer == 0 ) gsl_error( "null block_long", __FILE__, __LINE__, GSL_EFAULT );
    ...
    long& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "block_long is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of block_long", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
