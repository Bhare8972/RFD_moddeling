...
namespace gsl {
   ...
   *
   * If you want to use a std::vector<long double> or similar object as if it were a gsl::vector_long_double object,
   * use the constructor that takes a stride argument. But use it with caution.
   * Changing the size of the original object does not change the size of the gsl::vector_long_double object; so it
   * may be safer to create gsl::vector_long_double objects in this way as needed.
   */
  ...
  public:
    ...
    explicit vector_long_double( size_t const n ){
      if( n > 0 ) ccgsl_pointer = gsl_vector_long_double_alloc( n );
      else { ccgsl_pointer = new gsl_vector_long_double; ccgsl_pointer->size = 0; ccgsl_pointer->data = 0; }
      // just plausibly we could allocate vector_long_double but not count
      try { count = new size_t; } catch( std::bad_alloc& e ){
	// try to tidy up before rethrowing
	if( n > 0 ) gsl_vector_long_double_free( ccgsl_pointer );
    ...
    vector_long_double( vector_long_double const& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_long_double is now shared.
    }
    /**
     * The copy constructor. This shares the vector_long_double. Use clone() if you want a full copy.
     * @param v The vector_long_double to copy.
     */
    vector_long_double( vector_long_double& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( v.count ){
      if( count != 0 ) ++*count; // vector_long_double is now shared.
    }
    // assignment operator
    /**
	...
	if( ccgsl_pointer != 0 ){
	  if( ccgsl_pointer->size > 0 ) gsl_vector_long_double_free( ccgsl_pointer );
	  else delete ccgsl_pointer; }
	delete count;
      }
      // Then copy
     ...
     * to allow you to construct from a std::vector<long double> or std::array<long double,size_t> object.
     ...
     * the size of the gsl::vector_long_double does not change. Stride is normally set to 1 and allows us
     ...
     */
    template<typename V> vector_long_double( V& v, size_t const stride = 1 ){
      size_t const n = v.size() / stride;
      ccgsl_pointer = static_cast<gsl_vector_long_double*>( malloc( sizeof( gsl_vector_long_double ) ) );
    ...
    // Allow possibility of assigning from gsl_vector_long_double without sharing
    /**
     * This function is intended mainly for internal use. It allows @c this
     * to point to a gsl_vector_long_double without the possibility deleting it when @c this
     ...
     * gsl::vector_long_double* argument to one that takes a gsl_vector_long_double* argument.
     * @param v The gsl_vector_long_double
     */
     ...
     */
    void reset(){ vector_long_double().swap( *this ); }
#ifdef __GXX_EXPERIMENTAL_CXX0X__
    /**
     * Move constructor.
     * @param v The vector_long_double to move.
     */
    vector_long_double( vector_long_double&& v ) : ccgsl_pointer( v.ccgsl_pointer ), count( nullptr ){
    ...
    vector_long_double& operator=( vector_long_double&& v ){
      vector_long_double( std::move( v ) ).swap( *this );
      return *this;
    }
#endif
    // != operator
  ...
  public:
     ...
     * a GSL error if the vector_long_double stride is not 1. However, new vector_long_double objects are always initialised
	...
	gsl_error( "vector_long_double does not have stride of size 1", __FILE__, __LINE__, GSL_EBADLEN );
#endif
    ...
    long double& operator[]( size_t const n ){
      ...
      // First check that iterator is initialised.
      if( ccgsl_pointer == 0 ){
  	gsl_error( "vector_long_double is null", __FILE__, __LINE__, exception::GSL_EFAULT );
  	return something;
      }
#ifndef GSL_RANGE_CHECK_OFF
      // Check that position make sense
      if( n >= size() ){
  	gsl_error( "trying to read beyond end of vector_long_double", __FILE__, __LINE__, exception::GSL_EINVAL );
  	return something;
      }
  ...
  public:
     ...
     */
    gsl_vector_long_double* get() { return ccgsl_pointer; }
    /**
     * Get the gsl_vector_long_double.
     * @return the gsl_vector_long_double
     */
    gsl_vector_long_double const* get() const { return ccgsl_pointer; }
    /**
    ...
    /**
     * Another C++ version of gsl_vector_long_double_const_view_array().
     * @param v An array of type long double
     * @param n The size of the vector_long_double
     * @return A vector_long_double
     */
    static vector_long_double const view_array( long double const* v, size_t n ){
      gsl_vector_long_double* w = static_cast<gsl_vector_long_double*>( malloc( sizeof( gsl_vector_long_double ) ) );
      *w = gsl_vector_long_double_const_view_array( v, n ).vector;
      return vector_long_double( w );
    }
    /**
     * Another C++ version of gsl_vector_long_double_const_view_array_with_stride().
     * @param base An array of type long double
     * @param stride The stride
     * @param n The size of the vector_long_double
     * @return A vector_long_double
     */
    static vector_long_double const view_array_with_stride( long double const* base, size_t stride, size_t n ){
      gsl_vector_long_double* w = static_cast<gsl_vector_long_double*>( malloc( sizeof( gsl_vector_long_double ) ) );
      *w = gsl_vector_long_double_const_view_array_with_stride( base, stride, n ).vector;
      return vector_long_double( w );
    }
    ...
    /**
     * Another C++ version of gsl_const_vector_long_double_subvector_with_stride().
     * @param i The offset
     * @param n The size
     * @return A subvector
     */
    vector_long_double const subvector( size_t i, size_t n ) const {
      gsl_vector_long_double* w = static_cast<gsl_vector_long_double*>( malloc( sizeof( gsl_vector_long_double ) ) );
      *w = gsl_vector_long_double_const_subvector( get(), i, n ).vector;
      return vector_long_double( w );
    }
    /**
     * Another C++ version of gsl_vector_long_double_const_subvector_with_stride().
     * @param i The offset
     * @param stride The stride
     * @param n The size
     * @return A subvector
     */
    vector_long_double const subvector_with_stride( size_t i, size_t stride, size_t n ) const {
      gsl_vector_long_double* w = static_cast<gsl_vector_long_double*>( malloc( sizeof( gsl_vector_long_double ) ) );
      *w = gsl_vector_long_double_const_subvector_with_stride( get(), i, stride, n ).vector;
      return vector_long_double( w );
    }
